...
`,E=(e,t,a)=>f(e,t,"MM/dd/yyyy"),S=({availableTimeslots:r,onDaySelected:n,selectedDay:o,locale:i,timezone:l})=>{const[c,s]=p([]);x((()=>{const a=[];r.map((r=>{if(!e(new Date(r.startTime)))throw new Error(`Invalid date for start time on slot ${r.id}`);if(!e(new Date(r.endTime)))throw new Error(`Invalid date for end time on slot ${r.id}`);return t(new Date(r.startTime))!==t(new Date(r.endTime))&&a.push(E(new Date(r.endTime),l)),a.push(E(new Date(r.startTime),l)),null})),s([...new Set(a)])}),[r,l]);return u.createElement(k,{showNeighboringMonth:!1,defaultView:"month",onClickDay:e=>{n(e)},showNavigation:!1,tileDisabled:e=>"month"===e.view&&!c.some((t=>t===E(e.date,l))),tileClassName:e=>c.some((t=>t===E(e.date,l)))?["day-tile","active-day-tile"]:"month"===e.view?"day-tile":null,value:o,activeStartDate:a(o),calendarType:"gregory"})},D=h("button")`
  padding: 16px;
  border: none;
  color: rgba(var(--text-color-rgb), 1);
